...
export default function CategoryNavSection() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-black text-slate-900 tracking-tight">EXPLORE CATEGORIES</h2>
        ...
        {categories.map((cat) => {
          ...
          return (
            <Link
              key={cat.id}
              to={`/category/${cat.id}`}
              className="p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all shadow-sm border bg-white text-slate-700 border-slate-200 hover:border-teal-400 hover:bg-teal-50/30"
            >
